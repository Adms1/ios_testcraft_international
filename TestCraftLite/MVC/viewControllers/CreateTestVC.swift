...
//        self.arrChapter.append(TempModel2)
       // btnChapter.sette("Chapters", for: .normal)
...
//        self.txtRemarks.delegate = self
        ...
        txtQueCount.attributedPlaceholder = NSAttributedString(string: "Total Questions",
                                                               attributes: [NSAttributedString.Key.foregroundColor: UIColor.lightGray])
...
//    }
    ...
    func validated() -> Bool {
        ...
        {
            if tagListView_QueCount.tagViews.count == 0 {
                self.view.makeToast("Please Select Chapter", duration: 3.0, position: .bottom)
                valid = false
            }else
            {
        ...
        else //if tagListView_QueCount.tagViews.count > 0
        {
           let totalDisplay = String(format: "%.0f", self.intTotalQue)
            ...
            {
                self.view.makeToast("Question limit according to your selection is \(totalDisplay)", duration: 5.0, position: .center)
