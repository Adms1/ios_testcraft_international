...
class TestListVC: UIViewController, ActivityIndicatorPresenter ,ISRewardedVideoDelegate{
...
extension TestListVC{
    ...
    func rewardedVideoDidFailToShowWithError(_ error: Error!) {
       ...
       // tblTestList.isHidden = true
        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "Question1VC") as? Question1VC
//        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "TestIntroVC") as? TestIntroVC
        vc?.intProdSeconds = Float(arrTestList1[isAddShowIndexPath].RemainTime) ?? 0
        //Bhargav Hide
        ////print(arrTestList1[buttonRow])
    //    vc?.delegate = self
        vc?.arrTestData = [arrTestList1[isAddShowIndexPath]]
        vc?.strTestID = "\(arrTestList1[isAddShowIndexPath].TestID)"
        vc?.strTestTitle = "\(arrTestList1[isAddShowIndexPath].TestPackageName ?? "")"
        vc?.strStudentTestID = "\(arrTestList1[isAddShowIndexPath].StudentTestID)"
        vc?.int_Total_Hint = Int(arrTestList1[isAddShowIndexPath].NumberOfHint) ?? 0
        vc?.int_Used_Hint = Int(arrTestList1[isAddShowIndexPath].NumberOfHintUsed) ?? 0
