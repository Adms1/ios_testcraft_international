...
let kAPPKEY1 = "100f13569"


class Question1VC: UIViewController, ActivityIndicatorPresenter ,ISRewardedVideoDelegate{
    ...
    var midOfTestShowAds:Bool = true
...
//
    ...
    func setupIronSourceSdk() {
        ...
        IronSource.initWithAppKey(kAPPKEY1, adUnits:[IS_REWARDED_VIDEO])
...
//            queCollectionNewView.scrollToItem(at: indexPath as IndexPath, at: .centeredHorizontally, animated: true)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Analytics.logEvent("TestStartScreen", parameters: nil)
        setupIronSourceSdk()
...
//        NotificationCenter.default.removeObserver(self, name: .reachabilityChanged, object: reachability)
    ...
    func reloadView() {
        ...
        print("creash when show ads two times",strSelectedIndex)



//        if strSelectedIndex == Int(arrTestData[0].TotalQuestions)! / 2
//        {
//           // midOfTestShowAds = false
//            self.isTimerRunning = false
//            UserDefaults.standard.set(intProdSeconds, forKey: "showAdsTimerValue")
//            timer.invalidate()
...
// "Resume?", message: "Are you sure you want to resume a test?"
        ...
        alert.addAction(UIAlertAction(title: "Yes", style: .default, handler: {(alert: UIAlertAction!) in
            ...
            UserDefaults.standard.setValue("startTest", forKey: "isTestStart")
...
//                    newTabAdsView.frame = CGRect(x: 0, y: self.view.frame.size.height - 99, width: self.view.frame.size.width, height: 99)
//                    self.view.addSubview(newTabAdsView)
//
//                    self.tabBarController?.tabBar.frame = CGRect(x: 0, y: 0, width: self.view.frame.size.width, height: 50)
//                    newTabAdsView.addSubview(self.tabBarController!.tabBar)
//
//                    let customeView = ISBannerView(frame: CGRect(x: 0, y:50 , width: self.view.frame.size.width, height: 50))
...
//                }
    ...
    @IBAction func InfoClicked(_ sender: AnyObject) {
        ...
        Analytics.logEvent("Test_Information", parameters: [
                "2002": "2002" as NSObject
                ])
    }
...
////print("Send continue ANS : ",json)
    ...
    @objc func HintClicked(sender: UIButton) {
        ...
        Analytics.logEvent("FlagClick", parameters: [
                "Flag": "2004" as NSObject
                ])
...
////print("not iPad")
    ...
    @IBAction func reportClicked(_ sender: AnyObject) {
        ...
        Analytics.logEvent("ReportanIssueLinkClick", parameters: [
                "Report_an_issue": "2005" as NSObject
                ])
        ...
        let action = UIAlertAction(title: "Cancel", style: .cancel, handler: {(alert: UIAlertAction!) in
            ...
            Analytics.logEvent("CancelButtonClick", parameters: [
                    "Cancel_Button": "2104" as NSObject
                    ])
        ...
        Report_alert.addAction(UIAlertAction(title: "Question has a problem", style: .destructive, handler: {(alert: UIAlertAction!) in
            ...
            Analytics.logEvent("QuestionhasaProblemClick", parameters: [
                    "Question_problem": "2101" as NSObject
                    ])
        ...
        Report_alert.addAction(UIAlertAction(title: "Answer has a problem", style: .destructive, handler: {(alert: UIAlertAction!) in
            ...
            Analytics.logEvent("AnswerhasaProblemClick", parameters: [
                    "Answer_problem": "2101" as NSObject
                    ])
        ...
        Report_alert.addAction(UIAlertAction(title: "Hint has a problem", style: .destructive, handler: {(alert: UIAlertAction!) in
            ...
            Analytics.logEvent("HinthasaProblemClick", parameters: [
                    "Hint_problem": "2103" as NSObject
                    ])
            //                self.arrQueAns[self.strSectionSelectedIndex].arr_section_que[self.strSelectedIndex].report_issue = "1"
            //                self.reloadView()
        }))
    ...
    @IBAction func nextQue(_ sender: AnyObject) {
        ...
        }else
        {
            ...
            Analytics.logEvent("NextClick", parameters: [
                    "Next_Click": "2007" as NSObject
                    ])
...
////print("SUBMIT ANS : ",json)
        ...
        func Pre_Submit_Test_Api()
        {
            ...
            Analytics.logEvent("SubmitIconClick", parameters: [
                    "Submit_Icon": "2003" as NSObject
                    ])
            self.viewSubmitPopup.isHidden = false
            showActivityIndicator()
...
//                    self.runProdTimer()
    ...
    func Que_Ans_Submit_Api()
    {
        ...
        Analytics.logEvent("SubmitTestButtonClick", parameters: [
                "Submit_Test": "2010" as NSObject
                ])
        showActivityIndicator()
        //        //        let UDID: String = UIDevice.current.identifierForVendor!.uuidString
        //        //        var crct = 0
...
//                    var strPercentage = ""
                    for value in arrData! {
                        ...
                        self.crct = value["Correct"].stringValue //arrData.value(forKey: "TransactionID") as! String
                        self.Incrct = value["Wrong"].stringValue //arrData.value(forKey: "TransactionID") as! String
                        self.NotAnswer = value["UnAnswered"].stringValue //arrData.value(forKey: "TransactionID") as! String
                        self.Total = value["TotalMarks"].stringValue //arrData.value(forKey: "TransactionID") as! StringTotalGetMarks
                        self.TotalGetMarks = value["TotalGetMarks"].stringValue //arrData.value(forKey: "TransactionID") as! String


                        self.strPercentage = value["Percentile"].stringValue //arrData.value(forKey: "TransactionID") as! String
                    }
...
extension Question1VC:UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout
{
    @objc func popupSectionHeading(_ gesture:UIGestureRecognizer)
    {
        ...
        Analytics.logEvent("SectionInstruction", parameters: [
                "Secttion_Instruction": "2009" as NSObject
                ])
...
////print("not iPad")
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        case UICollectionView.elementKindSectionHeader:
            ...
            userHeader.imgDerctionIcon.isHidden = true
...
//                    cell.imgQueAns.sd_setIndicatorStyle(.white)
                    ...
                    DispatchQueue.main.asyncAfter(deadline: .now() + que_delay) {
                        ...
                        let url = URL.init(string: API.QueImg + (self.arrQueAns[self.strSectionSelectedIndex].arr_section_que[self.strSelectedIndex].que_ans[indexPath.row].img_link))
...
////print("not iPad")
        ...
        else
        {
            ...
            Analytics.logEvent("SelectOption", parameters: [
                    "Select_Option": "2006" as NSObject
                    ])
...
//        if available == false
//        {
//            if midOfTestShowAds == true
//            {
//                midOfTestShowAds = false
//                let timeValue =  UserDefaults.standard.float(forKey: "showAdsTimerValue")
//                print("after ads video show time",timeValue)
//                intProdSeconds = timeValue
//                print("update after ads video show time",timeValue)
//                self.runProdTimer()
//            }
//
//        }
...
//        }
        ...
        let vc = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(withIdentifier: "HighlightsVC") as? HighlightsVC
        vc!.strCorrect = "\(self.crct)"
        vc!.strInCorrect = "\(self.Incrct)"
        vc!.strUnanswered = "\(self.NotAnswer)"
        vc!.strTotal = "\(self.Total)"
        vc!.strTotalGetMarks = "\(self.TotalGetMarks)"
        vc!.strTestPercentage = "\(self.strPercentage)"
        vc!.strTestID = self.strTestID
        vc!.strStudentTestID = self.strStudentTestID
        vc!.strTitle = self.strTestTitle
        self.navigationController?.pushViewController(vc!, animated: true)
