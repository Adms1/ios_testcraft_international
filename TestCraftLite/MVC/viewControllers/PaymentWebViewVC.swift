...
class PaymentWebViewVC: UIViewController, ActivityIndicatorPresenter, SuccessFailedDelegate, UIWebViewDelegate,WKNavigationDelegate,WKUIDelegate//, WKUIDelegate, WKScriptMessageHandler, WKNavigationDelegate
{
    ...
    override func viewDidLoad() {
        ...
        let options: [String:Any] = [
            "amount": "\(amount!)",
            "currency": "USD",//"INR",//We support more that 92 international currencies.
           // "description": "test",//description,//"purchase description",
            "order_id": Order_ID!,//"order_4xbQrmEoA5WJ0G",
            "image": UIImage(named: "AppIcon") as Any,//"https://url-to-image.png",
            "name": "\(result.value(forKey: "StudentFirstName") ?? "")",//"business or product name",
            "prefill": [
                "contact": "\(result.value(forKey: "StudentMobile") ?? "")",//"9797979797",
                "email": "\(result.value(forKey: "StudentEmailAddress") ?? "")",//"foo@bar.com"
            ],
            "notes": [
                "mobile-orderid":Order_ID!,
                "categoryid":"TestCraftInternational",
                "device_type": "ios"
            ],
            "theme": [
                "color": "#3ea7e0"
    ...
    func Update_Payment_Api_packeage(status:String, payment_id: String)
    // func Update_Payment_Api(orederID:String,PaymentStatus:String,transaction_id:String)
    {
      ...
      //  print("success responce",dic)
        ...
        if (status == "failed"){
            ...
            params = ["PaymentOrderID":Order_ID!,"ExternalTransactionStatus":status, "ExternalTransactionID":"", "StudentID":"\(result.value(forKey: "StudentID") ?? "")"]

        }else{
            params =  ["PaymentOrderID":Order_ID! ,"ExternalTransactionStatus":status, "ExternalTransactionID":payment_id , "StudentID":"\(result.value(forKey: "StudentID") ?? "")"]
        }
    ...
    func Update_Payment_Api(status:String,payment_id:String)
    // func Update_Payment_Api(orederID:String,PaymentStatus:String,transaction_id:String)
    {
...
//    }
//}
extension PaymentWebViewVC : RazorpayPaymentCompletionProtocol {
...
//        if self.setrootViewController == "MyPackageListVCViewController" {
//            self.Update_Payment_Api_packeage(status: "failed", payment_id: "")
//        }else{
//            self.Update_Payment_Api(status: "failed", payment_id: "")
//        }
...
//                }
    ...
    func onPaymentSuccess(_ payment_id: String) {
        ...
        if self.setrootViewController == "MyPackageListVCViewController" {
            self.Update_Payment_Api_packeage(status: "success", payment_id: payment_id)
        }else{
            self.Update_Payment_Api(status: "success", payment_id: payment_id)
        }
    }
}
//extension PaymentWebViewVC: RazorpayPaymentCompletionProtocolWithData {
//
