...
//                setupTempleteListDropDown()
    ...
    @IBAction func CreateTestClicked(_ sender: AnyObject) {
        if validated() == true
        {
            ...
            Analytics.logEvent("CreateTestClick", parameters: nil)
...
//            else
            ...
            {
                self.view.makeToast("Max. duration is \(self.MaxQueTime) minutes", duration: 5.0, position: .center)
                valid = false
            }
        }
        return valid
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView?
            {
                ...
                headerView.topbgView1.backgroundColor = GetColor.themeBlueColor
//        GetColor.blueHeaderText
                ...
                // mehul hide this label
                headerView.lblNoOfQue.isHidden = true
//                headerView.lblNoOfQue.text = ""// "\(arrTemplete[indexPath.row].NoOfQue ?? "")"
                headerView.lblMarks.text = "No. * Marks"// "\(arrTemplete[indexPath.row].Marks ?? "")"
...
//                headerView.btnExpand.tag = section
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblSectionname.text = "\(arrTempleteDetail[indexPath.row].SubjectName ?? "")"// (\(arrTempleteDetail[indexPath.row].SubjectName ?? ""))"
        cell.lblQuestionTypeName.text = "\(arrTempleteDetail[indexPath.row].QuestionTypeName ?? "")"
        cell.lblNoOfQue.isHidden = true
//        cell.lblNoOfQue.text = "\(arrTempleteDetail[indexPath.row].NoOfQue ?? "")"
        cell.lblMarks.text = "\(arrTempleteDetail[indexPath.row].NoOfQue ?? "") * \(arrTempleteDetail[indexPath.row].Marks ?? "")"
