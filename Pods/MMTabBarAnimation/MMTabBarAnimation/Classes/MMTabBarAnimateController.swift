...
//        self.initAnimateItem()
    ...
    open override func viewDidLayoutSubviews() {
        ...
        self.tabBar.frame = CGRect(x: 0, y: view.frame.size.height-98, width: view.frame.size.width, height: 49)
       // self.resetBarItem()
    }
...
extension MMTabBarAnimateController {
    ...
    fileprivate func resetBarItem() {
        if let classType = NSClassFromString("UITabBarButton") {
            ...
            }).sorted{ $0.frame.origin.x < $1.frame.origin.y }

           // tabBar.fr

            tabBarSubView.enumerated().forEach({ (idx,view) in
                animateItems[idx].item = tabBar.items?[idx]
//                view.frame = CGRect(x: 0, y: view.frame.size.height-50, width: view.frame.size.width, height: 50)
//                animateItems[idx].tabBarView?.backgroundColor = .red
//                animateItems[idx].tabBarView?.frame = view.frame
